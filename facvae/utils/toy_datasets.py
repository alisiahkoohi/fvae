"""Set of 2D toy datasets.

Obtained from https://github.com/bayesiains/nsf/blob/master/data/plane.py
"""
...
class FourCirclesDataset(PlaneDataset):
    ...
    def __init__(self, num_points, flip_axes=False):
        if num_points % 4 != 0:
            raise ValueError(
                'Number of data points must be a multiple of four')
...
class DiamondDataset(PlaneDataset):
    ...
    def _create_data(self, rotate=True):
        ...
        index = np.random.choice(range(self.width**2),
                                 size=self.num_points,
                                 replace=True)
        noise = np.random.randn(self.num_points, 2)
        self.data = means[index] + noise @ covariance_factor
        if rotate:
            rotation_matrix = np.array([[1 / np.sqrt(2), -1 / np.sqrt(2)],
...
class TwoSpiralsDataset(PlaneDataset):
    ...
    def _create_data(self):
        n = torch.sqrt(torch.rand(
            self.num_points // 2)) * 540 * (2 * np.pi) / 360
...
class CheckerboardDataset(PlaneDataset):
    ...
    def _create_data(self):
        ...
        x2_ = torch.rand(self.num_points) - torch.randint(
            0, 2, [self.num_points]).float() * 2
...
class TwoMoonsDataset(PlaneDataset):
    ...
    def _create_data(self):
        data = make_moons(n_samples=self.num_points, noise=0.1,
                          random_state=0)[0]
