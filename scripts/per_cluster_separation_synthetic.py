...
def optimize(args, x_dataset, x_obs , glitch_idx, snippets_dataset_idx, glitch_dataset_idx):
    ...
    save_exp_to_h5(os.path.join(checkpointsdir(
        args.experiment), 'reconstruction_' + str(glitch_idx) + '.h5'),
                ...
                snippets_dataset_idx=snippets_dataset_idx,
                glitch_dataset_idx=glitch_dataset_idx,
                x_hat=x_hat)
...
if __name__ == '__main__':
    ...
    # from IPython import embed; embed()
    glitch, glitch_idx = snippet_extractor.waveforms_per_scale_cluster(
        vae_args, cmd_args.cluster_g, cmd_args.scale_g, sample_size=3)
    ...
    for j in range(glitch.shape[0]):
        ...
        snippets, snippets_idx = snippet_extractor.waveforms_per_scale_cluster(
            vae_args,
            cmd_args.cluster_n,
            cmd_args.scale_n,
            sample_size=cmd_args.R)
        snippets = snippets[:, 0, 0:1, :].astype(np.float64)
        optimize(cmd_args, snippets, g, j, snippets_idx, glitch_idx)
