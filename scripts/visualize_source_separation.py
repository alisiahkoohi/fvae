...
def plot_result(args, experiment_results):
    ...
    for filename, experiment in experiment_results.items():
        ...
        plt.savefig(
            os.path.join(
                plotsdir(
                    os.path.join(
                        experiment["args"].experiment, SAVE_DIR, filename[:-3]
                    )
                ),
    ...
    fig = plt.figure(figsize=(5, 1.5))
    plt.plot_date(
        time_intervals, x_obs_arr, color="#000000", lw=0.6, alpha=0.8, fmt=""
    )
    ax = plt.gca()
    ax.grid(True)
    ax.xaxis.set_major_locator(
    ...
    fig = plt.figure(figsize=(5, 1.5))
    plt.plot_date(
        time_intervals, x_hat_arr, color="#000000", lw=0.6, alpha=0.8, fmt=""
    )
    ax = plt.gca()
    ax.grid(True)
    ax.xaxis.set_major_locator(
