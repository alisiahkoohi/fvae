...
class Visualization(object):
    ...
    def __init__(self, args, network, dataset, data_loader, device):
        ...
        self.in_shape = {
            scale: dataset.shape["scat_cov"][scale] for scale in self.scales
        }
    ...
    def get_waveform(self, window_idx, scale):
        window_time_interval = self.get_time_interval(
            window_idx, scale, lmst=False
        )
        ...
        data_stream = data_stream.merge(
            method=MERGE_METHOD, fill_value=FILL_VALUE
        )
    ...
    def evaluate_model(self, args, data_loader):
        ...
        value_idx_map = {int(idx): i for i, idx in enumerate(argsorted_indices)}
        ...
        for _, idx in enumerate(data_loader):
            ...
            for scale in self.scales:
                ...
                latent_features[scale][np.sort(idx), :, :] = (
                    output["mean"][scale]
                    .reshape(
                        len(idx),
                        self.dataset.data["scat_cov"][scale].shape[1],
                ...
                for i in idx:
                    label = all_labels[scale][
                        argsorted_indices[value_idx_map[int(i)]]
                    ]
                    if label:
                        window_labels[scale][int(i)] = label
                    drop = all_drops[scale][
                        argsorted_indices[value_idx_map[int(i)]]
                    ]
                    if drop:
        ...
        for scale in self.scales:
            ...
            # return the indices.
            confident_idxs[scale] = torch.argsort(
    ...
    def load_per_scale_per_cluster_waveforms(
        ...
        self.waveforms = {
            scale: {str(i): [] for i in cluster_idx} for scale in scale_idx
        }
        ...
        def load_serial_job(shared_in, i):
            ...
            for sample_idx in range(
                ...
                window_idx = per_cluster_confident_idxs[scale][str(i)][
                    sample_idx
                ]
    ...
    def plot_fourier(self, args):
        ...
        def fourier_serial_job(shared_in, clusters):
            ...
            for cluster in clusters:
                print(
                    "Plotting Fourier transforms for cluster {}".format(cluster)
                )
                for scale in scales:
                    for sample_idx, waveform in enumerate(
                        ...
                        for comp in range(waveform.shape[0]):
                            ...
                            freqs = np.fft.fftfreq(
                                waveform.shape[1], d=1 / SAMPLING_RATE
                            )
        ...
        worker_in = np.array_split(
            np.arange(args.ncluster), args.ncluster, axis=0
        )
        with WorkerPool(
        ...
        for cluster in tqdm(range(args.ncluster), desc="Cluster loop"):
            for scale in tqdm(self.scales, desc="Scale loop", leave=False):
                split_idxs = np.array_split(
                    np.arange(
                        len(
                            self.per_cluster_confident_idxs[scale][str(cluster)]
                        )
                    ),
                    num_workers,
                    axis=0,
    ...
    def centroid_waveforms(self, args):
        ...
        """
        # sns.set_style("darkgrid")
        font = {"family": "serif", "style": "normal", "size": 28}
        matplotlib.rc("font", **font)
            ...
            for cluster in tqdm(
                ...
                for i in range(waves.shape[0]):
                    for j in range(waves.shape[1]):
                        waves[i, j, :] = waves[i, j, :] - np.mean(
                            waves[i, j, :]
                        )
                        waves[i, j, :] = waves[i, j, :] / np.std(waves[i, j, :])
                ...
                for i in range(1, waves.shape[0]):
                    ...
                    for j in range(waves.shape[1]):
                        ...
                        corr_coefs[i] += np.ma.corrcoef(
                            np.ma.masked_where(
                                np.isnan(bs_waveform[j, :]), bs_waveform[j, :]
                            ),
                            np.ma.masked_where(
                ...
                num_waveforms = 10
                dy = 1.1
                largest_corr = np.argsort(corr_coefs, axis=0)[::-1][
                    :num_waveforms
                ]

                fig, ax = plt.subplots(
                    1, centroid_waveforms.shape[0], figsize=(18, 6), sharey=True
                )
                ...
                for i in range(rolled_waveforms.shape[1]):
                    for j in range(
                        ...
                        normalized_rolled = (
                            normalized_rolled
                            / np.ma.max(np.ma.abs(normalized_rolled))
                            - j * dy
                        )
                        ax[i].plot(
                            np.linspace(
    ...
    def reconstruct_vae_input(self, args, sample_size=5):
        ...
        x = {
            scale: self.dataset.unnormalize(
                x[scale],
                "scat_cov",
                dset_name=scale,
            ).numpy()
        ...
        for scale in tqdm(self.scales, desc="Scale loop"):
            ...
            for i in range(sample_size):
                ...
                axes[0].set_title(
                    "fVAE reconstruction at scale {}".format(
                        SCALE_TO_TIME[scale]
                    )
                )
    ...
    def plot_latent_space(self, args):
        ...
        if os.path.exists(pre_computed_umap_file):
            ...
            for scale in self.scales:
                tmp_filename = os.path.join(
                    datadir("tmp"), "latent_features_" + scale + ".h5"
                )
            ...
            with WorkerPool(
                ...
                pool.map(
                    ...
                    list(
                        zip(
                            [1, 2, 3, 1],
                            self.scales,  # TODO: fix this. GPU ids are hardcoded.
            ...
            for scale in self.scales:
                filename = os.path.join(
                    datadir("tmp"), "umap_features_" + scale + ".h5"
                )
        ...
        for scale in tqdm(self.scales[-1:], desc="Scale loop"):
            ...
            (outlier_points, centered_points) = (
                detect_outliers_and_centered_points(
            ...
            centered_points = [label_idx[scale][idx] for idx in centered_points]
            ...
            for i in range(len(outlier_points)):
                ...
                for j in range(3):
                    outlier_waveforms["waveform"][i][j, :] = outlier_waveforms[
                        "waveform"
                    ][i][j, :] / np.linalg.norm(
                    ...
                    axes[j].set_ylim(
                        [
                            min(
                                outlier_waveforms["waveform"][i][j, :].reshape(
                                    -1
                                )
                            ),
                            max(
            ...
            for i in range(len(centered_points)):
                ...
                for j in range(3):
                    center_waveforms["waveform"][i][j, :] = center_waveforms[
                        "waveform"
                    ][i][j, :] / np.linalg.norm(
                    ...
                    axes[j].set_ylim(
                        [
                            min(
                                center_waveforms["waveform"][i][j, :].reshape(
                                    -1
                                )
                            ),
                            max(
        ...
        for window_feature, window_marker in zip(["label", "drop"], ["*", "o"]):
            for scale in self.scales:
                ...
                if window_feature == "label":
                    custom_legend = plt.Line2D(
                        ...
                        color="w",
                        label=(
                            args.event_type[0].capitalize()
                            + " events ("
                            + str(len(label_idx[scale]))
                            + " windows)"
                        ),
                ...
                plt.title(
                    "Latent samples at scale {}".format(SCALE_TO_TIME[scale])
                )
    ...
    def plot_scatspec_umap(self, args):
        ...
        args.umap_n_neighbors = (
            100  # For memory reasons, we use a smaller number of neighbors.
        )
